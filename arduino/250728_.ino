...
void serialStream()
{
	while (Serial.available() != 0)
	{
		...
		//   case 23:
		//     needleBed[messageCounter++] = currentMessage;
		//     if (messageCounter >= currentMessage) {
		//       messageCounter = currentMessage = 0;
		//     }
		// case 23:
		// Serial.println()
		// }
...
void needlePosition(int pinNumber, int intIndex)
{
	...
	{ // Breytum bara ef við erum á nálaborðinu.
		...
		// Serial.println(intIndex);
		// Serial.println(bitIndex);
		// Serial.println(byteIndex);
		// Serial.println((needleBed[byteIndex] & (1 << (bitIndex))) > 0);
		// int bitIndex = (intIndex+1) % 8;
		// bool multipleOf8 =  (byteIndex*intIndex) == 0;
		// int multipleOf8 = (byteIndex*intIndex) == 0 ?
